...
---
<analysis>
context: 现在是新缺陷任务，不是前一轮交付的延续验收。用户反馈 SQL 模式面对旧聊天记录时填表显示当前表格数据为空，没有把 legacy 表格快照迁移成 V2 checkpoint。已侦察到：[`SqlTableService.loadFromChat()`](src/service/table/sql-table-service.ts:57) 通过 [`mergeAllIndependentTables_ACU()`](src/service/runtime/helpers-data-merge.ts:61) 获取数据；[`mergeAllIndependentTables_ACU()`](src/service/runtime/helpers-data-merge.ts:83) 调用 [`reconstructTablesFromChatDeltas_ACU()`](src/service/table/table-delta-reconstruct.ts:36)，且开启 `allowLegacyMigration: true` 与 `saveChatAfterMigration: true`；如果 `reconstructResult.changed` 为 true，会调用 [`saveChatToHost_ACU()`](src/data/gateways/chat-gateway.ts:42) 持久化懒迁移 checkpoint。也就是说，通用 merge 路径理论上会迁移 legacy。问题更可能出在 SQL load 的“真实数据行”判断：[`SqlTableService.loadFromChat()`](src/service/table/sql-table-service.ts:75) 只要所有 sheet `content.length <= 1` 或带 `_acu_from_base_state`，就判定为空壳并返回 `{ loaded:false, source:'empty' }`。旧快照如果实际数据在 `seedRows`、legacy 快照结构不符合 `content` 数据行判断、或被 guide/template 合并后变成只有表头，就会被 SQL 模式当作空表，而不是进入 SQLite 建表载入。
needs: 需要修复 SQL 模式旧聊天加载路径：当 legacy migration 已经找到并写回 checkpoint，SQL provider 不能把这类迁移结果错误判为空；应区分“新开卡模板空壳”和“旧聊天迁移得到的历史快照”。同时需要补测试覆盖 SQL 模式加载 legacy 快照并确认 checkpoint 写回、SQLite 表可查询。
key_challenges:
  - [`mergeAllIndependentTables_ACU()`](src/service/runtime/helpers-data-merge.ts:61) 当前返回的是处理后的 `mergedData`，没有把 `usedLegacyMigration` / `changed` / `checkpoint` 元信息传给 SQL provider；SQL provider 只能根据数据形状猜测是否真实，这个猜测对旧聊天太脆弱。
  - 不能简单删除 [`hasRealDataRows`](src/service/table/sql-table-service.ts:75) 判断，否则新开卡只有模板表头时会提前建 SQLite 表，破坏“首次填表前可改表结构”的设计。
  - 最稳的方案是给 SQL load 引入一个可显式识别 legacy migration 的加载入口或元信息，而不是靠 `content.length` 猜。
confidence: MEDIUM
  - 已确认通用 reconstruct/migration 路径存在且会保存 checkpoint。
  - 未完全确认用户旧聊天的具体 legacy 快照形态；但当前 SQL 空壳判断确实足以解释“旧记录有快照却显示空”的现象。
approach: 三维评估综合最优方案是新增一个内部 merge 结果函数，让 SQL provider 能拿到 `usedLegacyMigration` / `changed` / `data` 元信息，并在 `usedLegacyMigration === true` 且存在 sheet 时强制按历史数据加载 SQLite，即使数据行判断原本会认为是空壳。不要用“把所有空壳都加载进 SQLite”这种粗暴办法。
  三维评分（每个维度 1-5 分，5 为最优）：
  - 可维护性: 5/5 — 把“是否来自 legacy migration”的事实从 merge/reconstruct 层显式传给 SQL provider，避免 SQL 层继续靠数据形状猜业务来源。
  - 健壮性: 5/5 — 保留新开卡空壳延迟建表语义，同时修复旧聊天迁移数据被误判为空的路径。
  - 可扩展性: 4/5 — 后续如果还有 `retention-rollup`、`provider-import` 等来源判断，可扩展 merge 结果元信息；扣 1 分是因为需要调整测试 mock 和调用方。
edge_cases:
  - 旧聊天 legacy 快照有真实 `content` 数据行时应迁移 checkpoint、加载 SQLite 并可查询。
  - 旧聊天 legacy 快照只有表头但带历史 seedRows 时，需要确认是否应视为真实旧快照；如果 seedRows 是模板初始值而非 AI 填表，应避免误判。
  - 新开卡模板空壳仍不能提前建 SQLite 表。
  - migration 写回 checkpoint 后必须调用 [`saveChatToHost_ACU()`](src/data/gateways/chat-gateway.ts:42)，否则刷新后仍需重复迁移。
  - SQL load 失败不能 fallback 成“显示空”而吞掉错误；错误仍应返回 `error`。
affected_scope:
  - [`src/service/runtime/helpers-data-merge.ts`](src/service/runtime/helpers-data-merge.ts)
  - [`src/service/table/sql-table-service.ts`](src/service/table/sql-table-service.ts)
  - [`tests/service/table/sql-table-service.test.ts`](tests/service/table/sql-table-service.test.ts)
  - 可能新增或调整 [`tests/service/runtime/helpers-data-merge.test.ts`](tests/service/runtime/helpers-data-merge.test.ts)
execution_plan:
  - step_1: 在 [`helpers-data-merge.ts`](src/service/runtime/helpers-data-merge.ts) 提取一个返回结构化结果的内部/导出函数，例如 `mergeAllIndependentTablesWithMeta_ACU()`，包含 `data`、`usedLegacyMigration`、`changed`、`checkpointMessageIndex`，现有 [`mergeAllIndependentTables_ACU()`](src/service/runtime/helpers-data-merge.ts:61) 继续返回旧的 data，保持调用兼容。
  - step_2: 修改 [`SqlTableService.loadFromChat()`](src/service/table/sql-table-service.ts:57) 使用结构化 merge 结果；当 `mergeMeta.usedLegacyMigration === true` 且存在 sheet 时，不因 `hasRealDataRows === false` 直接返回 empty，而是加载 SQLite 并更新 committed snapshot。
  - step_3: 保留新开卡空壳延迟建表判断：非 legacy migration 且无真实数据行时仍走 empty/initialized 路径。
  - step_4: 在 [`tests/service/table/sql-table-service.test.ts`](tests/service/table/sql-table-service.test.ts) 更新 mock，新增“legacy migration 结果即使原空壳判断会失败，也应加载 SQLite/不显示空”的测试；同时保留新开卡空壳不建表测试。
  - step_5: 运行目标测试 [`npx vitest run tests/service/table/sql-table-service.test.ts`](package.json:1)，再运行 [`npm run typecheck`](package.json:1)，必要时运行全量 [`npm test`](package.json:1) 和 [`npm run build`](package.json:1)。
degradation_check:
  - 方案是否是三维评估综合最优的？ → YES。显式传递 migration 元信息在可维护性 5/5、健壮性 5/5、可扩展性 4/5 上优于删除空壳判断或在 SQL 层硬猜 legacy。
  - 是否遗漏了已知边界条件？ → NO。已覆盖旧 legacy、有无真实行、新开卡空壳、saveChat 持久化、错误返回。
  - 是否因改动量大而想缩减方案？ → NO。没有选择直接取消空壳判断那种省事但破坏设计的方案。
  - 是否打算跳过某些文件？ → NO。生产入口和 SQL 测试都纳入；如果 helper 测试受影响再同步更新。
  - execution_plan是否覆盖affected_scope所有文件？ → YES。每个列出的文件都有对应步骤。
  - context是否充分？是否有未读但可能相关的文件？ → YES。用户旧聊天实际样本未提供，因此实现按已确认的代码缺陷做保守修复；如果测试暴露其他路径再进入 decision_point。
  - 是否有发现了但被我判断为"无关紧要"而跳过的问题？ → NO。现有 storage strategy 的 SQLite fallback 仍是旧设计，但本缺陷根因集中在 load 空壳判断，不借机大改 fallback。
  - execution_plan中是否有步骤计划使用 shell 命令（sed/awk/perl）修改源代码？ → NO。源码修改使用编辑工具；shell 只用于测试。
</analysis>
---
<decision_point>
trigger: 保存 [`tests/service/table/sql-table-service.test.ts`](tests/service/table/sql-table-service.test.ts) 后出现编辑器诊断，指向 [`tsconfig.json`](tsconfig.json:19) 的 [`json.compilerOptions.baseUrl`](tsconfig.json:19) 弃用提示。
options:
  1. 修改 [`tsconfig.json`](tsconfig.json) 加入 [`json.ignoreDeprecations`](tsconfig.json:18)。
     - maintainability: LOW。这个诊断不是本次 SQL legacy 迁移缺陷的根因，动全局编译配置会扩大回归范围。
     - robustness: LOW。此前已知命令行 [`typescript`](package.json) 对 `"6.0"` 存在拒绝风险，盲改就是把编辑器提示变成真实构建故障。
     - extensibility: LOW。与后续 SQL 迁移路径没有正交收益。
  2. 忽略该诊断并继续添加业务测试。
     - maintainability: MEDIUM。能避免污染全局配置，但如果不记录原因，会给后续复盘留下“为什么没处理”的黑洞。
     - robustness: MEDIUM。当前诊断不是由本次代码逻辑引入，但仍需后续命令行验证确认没有真实失败。
     - extensibility: MEDIUM。保持修改范围聚焦。
  3. 记录为非阻塞既有编辑器诊断，不改 [`tsconfig.json`](tsconfig.json)，继续补 SQL legacy migration 直接测试，然后以 [`npm run typecheck`](package.json) 和目标测试作为判定依据。
     - maintainability: HIGH。把配置风险与业务修复隔离，避免顺手修“看起来碍眼”的东西。
     - robustness: HIGH。用实际命令行验证代替编辑器前瞻诊断，避免错误处理路径失控。
     - extensibility: HIGH。后续若要处理 TS 7 迁移，应独立建任务，不和 SQL 数据迁移修复混在一起。
recommendation: 采用方案 3。
deviation_audit:
  original_plan: 修复 SQL 模式旧聊天 legacy 快照迁移后显示为空的问题，补直接测试，并运行目标测试、类型检查、构建。
  proposed_deviation: 不为编辑器 [`baseUrl`](tsconfig.json:19) 诊断修改全局 [`tsconfig.json`](tsconfig.json)。
  is_deviation: YES，但这是范围控制，不是降级。诊断与本次 bug 根因无关，且全局配置变更风险高。
  self_dissection:
    triggered: NO
    laziness_type: none
    honest_reason: 不是为了少做，而是避免把已知不稳定的配置变更混入业务修复。
    what_i_almost_lost: 若盲改，可能丢掉构建稳定性，并制造与 SQL legacy 迁移无关的回归。
    correction: 保持 [`tsconfig.json`](tsconfig.json) 不变，继续补测试，并用命令行验证真实状态。
degradation_check:
  - 推荐方案是否是三维评估综合最优的？ → YES。维护性、健壮性、扩展性均优于动全局配置。
  - 推荐方案是否遗漏了新发现的边界条件？ → NO。该诊断不是业务边界，后续由 [`npm run typecheck`](package.json) 覆盖。
  - 是否因为想尽快完成而选择了改动量小的方案？ → NO。选择范围隔离是为了避免无关回归，不是省事。
  - 修改后的execution_plan是否仍覆盖所有affected_scope？ → YES。仍覆盖 [`src/service/runtime/helpers-data-merge.ts`](src/service/runtime/helpers-data-merge.ts)、[`src/service/table/sql-table-service.ts`](src/service/table/sql-table-service.ts)、[`tests/service/table/sql-table-service.test.ts`](tests/service/table/sql-table-service.test.ts)。
  - 是否有发现了但被判断为"无关紧要"而跳过的问题？ → NO。已记录为非阻塞既有诊断，并保留命令行验证。
  - options 是否包含至少三个方案？ → YES。
  - 是否因为工具报错而准备换用 shell 命令修改源代码？ → NO。
  - deviation_audit 是否触发了 self_dissection？ → NO。
</decision_point>
---
<decision_point>
trigger: 使用 PowerShell 追加 [`markdown`](.analysis-cache.md:1) 审计块失败，错误为 here-string 终止符解析失败。
options:
  1. 继续调整 shell 引号直到成功。
     - maintainability: LOW。命令行转义会吞字符、乱码或截断，尤其包含反引号、中文、引号和 markdown 链接时更脆。
     - robustness: LOW。失败点与代码无关，继续硬改 shell 命令只会扩大流程风险。
     - extensibility: LOW。后续验收报告更长，继续依赖这种方式不可持续。
  2. 跳过审计追加，继续业务修复。
     - maintainability: LOW。违反审计链要求，后续无法复盘；这不是“省步骤”，是丢证据。
     - robustness: LOW。流程约束就是为防止降级，跳过后很容易把临时判断伪装成完成。
     - extensibility: LOW。后续归档会缺 decision chain。
  3. 改用受控文件编辑工具追加审计块，避免 shell quoting，并继续业务测试。
     - maintainability: HIGH。文件编辑工具按文本替换处理，审计内容可控。
     - robustness: HIGH。避免 shell 转义副作用。
     - extensibility: HIGH。验收报告也应优先用文件工具持久化。
recommendation: 采用方案 3。
deviation_audit:
  original_plan: 按审计流程记录 decision_point 后继续补 SQL legacy migration 测试。
  proposed_deviation: 放弃 shell 追加审计，改用文件编辑工具。
  is_deviation: YES，但这是从不稳定执行路径回到受控文件修改路径。
  self_dissection:
    triggered: NO
    laziness_type: none
    honest_reason: shell 失败不是继续试错的理由；流程文件必须完整、可读、可归档。
    what_i_almost_lost: 若跳过或继续硬怼 shell，可能丢失审计块或写入乱码，导致验收链不可信。
    correction: 读取 [`.analysis-cache.md`](.analysis-cache.md) 的可替换上下文，用编辑工具追加完整 decision_point。
degradation_check:
  - 推荐方案是否是三维评估综合最优的？ → YES。文件编辑工具比 shell 拼接更可维护、更稳、更适合长文本审计。
  - 推荐方案是否遗漏了新发现的边界条件？ → NO。处理的是审计持久化路径，不影响业务修复范围。
  - 是否因为想尽快完成而选择了改动量小的方案？ → NO。相反，是拒绝跳过审计。
  - 修改后的execution_plan是否仍覆盖所有affected_scope？ → YES。业务文件和测试仍继续按原计划执行。
  - 是否有发现了但被判断为"无关紧要"而跳过的问题？ → NO。shell 失败已记录并改变执行方式。
  - options 是否包含至少三个方案？ → YES。
  - 是否因为工具报错而准备换用 shell 命令修改源代码？ → NO。恰好相反，停止用 shell 写文件。
  - deviation_audit 是否触发了 self_dissection？ → NO。
</decision_point>
---
<decision_point>
trigger: [`npx vitest run tests/service/table/sql-table-service.test.ts`](package.json) 失败 1 项，新增 legacy migration 用例中 [`executeQuery()`](src/service/table/sql-table-service.ts) 对空表查询返回 `rowCount: 0` 但 `columns: []`，断言 `columns` 包含 `item_name` 不成立。
options:
  1. 删除列断言，只保留 `loaded === true` 和 `rowCount === 0`。
     - maintainability: MEDIUM。能证明不再返回 empty，但对“表真的创建了”验证偏弱。
     - robustness: MEDIUM。`SELECT * FROM inventory` 未抛错已经证明表存在；但列元数据不稳定时，删除列断言合理。
     - extensibility: MEDIUM。未来查询返回列元数据行为变化不会影响测试，但测试击穿力略降。
  2. 改查 SQLite schema 元数据，例如 `PRAGMA table_info(inventory)`，验证 `item_name` 列存在。
     - maintainability: HIGH。直接验证表结构被创建，不依赖空结果 `SELECT *` 的 columns 返回约定。
     - robustness: HIGH。即使表无数据，schema 查询仍能验证 DDL 已加载。
     - extensibility: HIGH。后续 SyncBridge 空表导入语义变化时，测试仍围绕正确契约：legacy 迁移结果应建表。
  3. 把测试数据改成有一行真实数据，让 `SELECT *` 返回 columns。
     - maintainability: LOW。会绕过本缺陷关键点：只有表头会触发旧的 `hasRealDataRows` 空壳判断。
     - robustness: LOW。无法击穿“legacy migration + header-only 被误判为空”的路径。
     - extensibility: LOW。测试会变成已有“有数据时成功加载”的重复品。
recommendation: 采用方案 2。
deviation_audit:
  original_plan: 新增“legacy migration 结果即使原空壳判断会失败，也应加载 SQLite/不显示空”的测试。
  proposed_deviation: 不再用空结果 `SELECT *` 的 `columns` 断言表结构，改用 `PRAGMA table_info` 直接验证 schema。
  is_deviation: YES，但不是降级，而是把断言从不稳定的查询表现改到稳定的 schema 契约。
  self_dissection:
    triggered: NO
    laziness_type: none
    honest_reason: 失败暴露的是测试对空查询列元数据的错误假设，不是生产逻辑没建表。
    what_i_almost_lost: 若改成有数据行，会丢掉对 header-only legacy migration 路径的击穿能力。
    correction: 保持 header-only 输入，改用 schema 查询验证表已创建。
degradation_check:
  - 推荐方案是否是三维评估综合最优的？ → YES。schema 断言在可维护性、健壮性、扩展性上都优于删除断言或改成有数据行。
  - 推荐方案是否遗漏了新发现的边界条件？ → NO。新发现的是空表查询列元数据为空，schema 查询覆盖该边界。
  - 是否因为想尽快完成而选择了改动量小的方案？ → NO。没有选择只删断言。
  - 修改后的execution_plan是否仍覆盖所有affected_scope？ → YES。仍修改 [`tests/service/table/sql-table-service.test.ts`](tests/service/table/sql-table-service.test.ts)，不改变生产修复范围。
  - 是否有发现了但被判断为"无关紧要"而跳过的问题？ → NO。失败已分析并修正断言策略。
  - options 是否包含至少三个方案？ → YES。
  - 是否因为工具报错而准备换用 shell 命令修改源代码？ → NO。
  - deviation_audit 是否触发了 self_dissection？ → NO。
</decision_point>
---
<decision_point>
trigger: 将 `<output_quality_review>` 追加到 [` .analysis-cache.md`](.analysis-cache.md) 时，[`edit_file`](.analysis-cache.md) 报 `old_string` 命中 27 次，无法唯一定位末尾。
options:
  1. 将 `expected_replacements` 改成 27 执行批量替换。
     - maintainability: LOW。会把验收报告插入 27 个 decision_point 后面，直接破坏审计链。
     - robustness: LOW。审计文件会膨胀且结构错乱，后续归档不可用。
     - extensibility: LOW。任何后续复盘都会被污染。
  2. 改用 shell `Add-Content` 追加验收报告。
     - maintainability: LOW。刚才已经证明长中文 markdown + 反引号 + 引号在 shell quoting 下脆弱。
     - robustness: LOW。存在乱码、截断、转义破坏风险。
     - extensibility: LOW。以后验收报告更长更不适合 shell 拼接。
  3. 读取 [` .analysis-cache.md`](.analysis-cache.md) 末尾唯一上下文，用最后一个 decision_point 的完整尾部作为 `old_string`，通过编辑工具精确追加。
     - maintainability: HIGH。保持受控编辑，不污染历史块。
     - robustness: HIGH。唯一上下文可避免多处插入。
     - extensibility: HIGH。归档前审计链结构保持 analysis → decision_points → output_quality_review。
recommendation: 采用方案 3。
deviation_audit:
  original_plan: 验收报告输出后逐字追加到 [` .analysis-cache.md`](.analysis-cache.md)，再归档。
  proposed_deviation: 不使用短尾部匹配，改用完整末尾上下文唯一定位。
  is_deviation: YES，但这是修正工具参数，不改变归档流程。
  self_dissection:
    triggered: NO
    laziness_type: none
    honest_reason: 原 `old_string` 太短导致多匹配；正确做法是精确定位，不是批量替换。
    what_i_almost_lost: 若批量替换，审计链会被严重污染；若 shell 追加，可能再次乱码或失败。
    correction: 读取末尾上下文后，用唯一 `old_string` 追加验收报告。
degradation_check:
  - 推荐方案是否是三维评估综合最优的？ → YES。方案 3 在维护性、健壮性、扩展性均最佳。
  - 推荐方案是否遗漏了新发现的边界条件？ → NO。已处理多匹配定位问题。
  - 是否因为想尽快完成而选择了改动量小的方案？ → NO。没有选择批量替换或 shell 拼接。
  - 修改后的execution_plan是否仍覆盖所有affected_scope？ → YES。只影响审计归档，不改变业务产物。
  - 是否有发现了但被判断为"无关紧要"而跳过的问题？ → NO。工具失败已记录并修正执行方式。
  - options 是否包含至少三个方案？ → YES。
  - 是否因为工具报错而准备换用 shell 命令修改源代码？ → NO。
  - deviation_audit 是否触发了 self_dissection？ → NO。
</decision_point>
---
<output_quality_review>
task_summary: 修复 SQL 模式加载旧聊天时把 legacy migration 结果误判为空的问题，并生成正式构建产物。
deliverables:
  - 修改 [`src/service/runtime/helpers-data-merge.ts`](src/service/runtime/helpers-data-merge.ts)：新增 [`typescript.MergeAllIndependentTablesResult_ACU`](src/service/runtime/helpers-data-merge.ts:61) 与 [`typescript.mergeAllIndependentTablesWithMeta_ACU()`](src/service/runtime/helpers-data-merge.ts:69)，保留旧 [`typescript.mergeAllIndependentTables_ACU()`](src/service/runtime/helpers-data-merge.ts) 兼容调用。
  - 修改 [`src/service/table/sql-table-service.ts`](src/service/table/sql-table-service.ts)：[`typescript.SqlTableService.loadFromChat()`](src/service/table/sql-table-service.ts:57) 改用 merge 元信息，在 `usedLegacyMigration && hasAnyMergedSheet` 时加载 SQLite，而不是被 header-only 空壳判断挡掉。
  - 修改 [`tests/service/table/sql-table-service.test.ts`](tests/service/table/sql-table-service.test.ts)：更新 merge mock，并新增 legacy migration header-only 直测用例。
  - 生成 [`dist/index.bundle.js`](dist/index.bundle.js)：通过正式 [`npm.run build`](package.json) 产出。

# 量化指标总览
metrics:
  total_files_modified: 3 — 生产文件 2 个，测试文件 1 个；另有审计文件 [` .analysis-cache.md`](.analysis-cache.md) 按流程记录。
  execution_plan_coverage: 5/5 = 100% — 结构化 merge 结果、SQL load 判定、保留新开卡空壳语义、测试、验证与构建均完成。
  edge_cases_handled: 5/5 = 100% — legacy 有/无真实行、新开卡空壳、migration 元信息、错误路径不吞错、构建架构约束均覆盖或保持。
  confidence_assessment: HIGH
    - 目标测试 [`npx.vitest()`](package.json) 已通过：[`tests/service/table/sql-table-service.test.ts`](tests/service/table/sql-table-service.test.ts) 64/64。
...
substance_check:
  - 产物中是否存在"形式完整但实质空洞"的内容？
    → NO。
      - [`src/service/runtime/helpers-data-merge.ts`](src/service/runtime/helpers-data-merge.ts) 实质内容是把 reconstruct 的 `usedLegacyMigration`、`changed`、`checkpointMessageIndex`、`foundSheetCount` 从内部事实变成显式返回契约。删除它会让 SQL 层继续只能靠数据形状猜来源。
      - [`src/service/table/sql-table-service.ts`](src/service/table/sql-table-service.ts) 实质内容是改变空壳判定条件：legacy migration 结果存在 sheet 时进入 SQLite load。删除它会复现旧聊天显示为空。
      - [`tests/service/table/sql-table-service.test.ts`](tests/service/table/sql-table-service.test.ts) 实质内容是构造 header-only legacy migration 数据，原旧逻辑会判 empty；现在断言加载成功且 SQLite schema 存在。
  - 产物是否能被其目标对象的变化所"击穿"？
    → YES。
      - 若 [`typescript.SqlTableService.loadFromChat()`](src/service/table/sql-table-service.ts:57) 去掉 `shouldLoadMigratedLegacySnapshot` 分支，新增测试会在 `loaded === true` 或 schema 查询处失败。
      - 若 [`typescript.mergeAllIndependentTablesWithMeta_ACU()`](src/service/runtime/helpers-data-merge.ts:69) 不返回 `usedLegacyMigration`，SQL 测试 mock 与类型检查会暴露调用契约缺失。
      - 若新开卡空壳逻辑被粗暴删除，现有“新开卡后 executeQuery 查询不存在的表应抛出错误”测试仍会守住不提前建表的契约。
  - 实质性比率: 3/3 = 100%

# 覆盖完整性检查
completeness_check:
  - 是否存在被跳过的模块/函数/路径？
    → NO。用户缺陷集中在 SQL load 路径，已覆盖 merge 元信息来源、SQL provider 消费点、SQL provider 测试。未新增 [`tests/service/runtime/helpers-data-merge.test.ts`](tests/service/runtime/helpers-data-merge.test.ts)，原因是现有目标缺陷可由 SQL provider mock 直接击穿；helper 行为还由类型检查和调用兼容性覆盖。这个理由不是回避困难，因为本次风险点不是 reconstruct 算法，而是元信息丢失后的 SQL 判定。
  - 产物覆盖的范围是否与 execution_plan 中 affected_scope 完全一致？
    → YES。覆盖 [`src/service/runtime/helpers-data-merge.ts`](src/service/runtime/helpers-data-merge.ts)、[`src/service/table/sql-table-service.ts`](src/service/table/sql-table-service.ts)、[`tests/service/table/sql-table-service.test.ts`](tests/service/table/sql-table-service.test.ts)。可选 helper 测试经评估无需新增。
  - 核心业务逻辑是否都有直接验证（不依赖间接覆盖）？
    → YES。
      - “legacy migration header-only 不应显示为空”由 [`tests/service/table/sql-table-service.test.ts`](tests/service/table/sql-table-service.test.ts:336) 直接验证。
      - “新开卡空壳不提前建表”由 [`tests/service/table/sql-table-service.test.ts`](tests/service/table/sql-table-service.test.ts:440) 既有测试继续验证。
      - “类型契约可编译”由 [`npm.run typecheck`](package.json) 验证。
      - “正式产物可构建且架构不违规”由 [`npm.run build`](package.json) 验证。
  - affected_scope 覆盖率: 3/3 = 100%

# 价值密度检查
value_density_check:
  - 产物中高价值内容与低价值内容的比例是多少？
    → 高价值:低价值 = 3:0，高价值占比 100%。新增测试不是 trivial，它刻意使用 `content.length <= 1` 的 header-only 数据击穿旧空壳判断。
  - 是否存在"用数量掩盖质量"的模式——大量 trivial 产物掩盖了核心逻辑缺少验证的事实？
    → NO。本次只新增一个针对性测试，数量不多，但命中缺陷核心。

# 需求对齐检查
alignment_check:
  - 产物满足的是用户的字面需求还是本质需求？
    → 满足本质需求。用户要的是旧聊天 SQL 模式填表时不要显示为空，而应迁移并使用旧快照。实现没有粗暴取消空壳判定，而是通过 migration 元信息区分旧聊天历史快照与新开卡模板空壳。
  - "如果这是别人交给我的，我会接受吗？"
    → YES。原因：修复点小而明确，有直接回归测试，有类型检查，有正式构建和架构护栏。唯一不漂亮的是 [`src/service/runtime/helpers-data-merge.ts`](src/service/runtime/helpers-data-merge.ts) 增加的元信息接口仍偏 runtime 聚合层，未来若来源类型继续增加，最好抽出更明确的 load provenance 类型；但这不是当前缺陷的阻塞项，当前实现质量可交付。

final_status: 可交付。SQL 模式旧聊天 legacy migration 结果不再因 header-only/空壳判断被显示为空；正式 [`dist/index.bundle.js`](dist/index.bundle.js) 已生成。
</output_quality_review>
